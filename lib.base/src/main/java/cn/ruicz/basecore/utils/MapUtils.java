...
public class MapUtils {
    ...
    public static void showNavDialog(Context context, double lat, double lng, String name){
        ...
        if (ObjectUtils.isEmpty(listData)) {
            ToastUtils.showLong("请安装导航软件");
            return;
        }
        new XPopup.Builder(context).asCenterList("导航-"+name, listData.toArray(new String[0]), (position, text) -> {
            if (TextUtils.equals(text, "高德地图")){
                openGaoDeNavi(context,0,0,null, lat,lng,name);
            }else if (TextUtils.equals(text, "腾讯地图")){
                openTencentMap(context,0,0,null, lat,lng,name);
            }else if (TextUtils.equals(text, "百度地图")){
                openBaiDuNavi(context,0,0,null, lat,lng,name);
            }
        }).show();
    }
