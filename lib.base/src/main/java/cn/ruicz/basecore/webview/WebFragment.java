...
import cn.ruicz.basecore.databinding.FragmentWebBinding;
import cn.ruicz.basecore.webview.client.MiddlewareChromeClient;
import cn.ruicz.basecore.webview.client.MiddlewareWebViewClient;
...
public class WebFragment extends BaseFragment<FragmentWebBinding, BaseViewModel> {
    ...
    public void init() {
        mAgentWeb = AgentWeb.with(this)//
                .setAgentWebParent(binding.contentLayout, -1, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT))//传入AgentWeb的父控件。
                .useDefaultIndicator(R.attr.colorPrimary, 3)//设置进度条颜色与高度，-1为默认值，高度为2，单位为dp。
...
//                .setNotifyIcon(R.drawable.ic_file_download_black_24dp) 4.0.0删除该api //下载通知图标。4.0.0后的版本请通过AgentWebDownloader#Extra修改icon
                .setOpenOtherPageWays(DefaultWebClient.OpenOtherPageWays.DERECT)//打开其他页面时，弹窗质询用户前往其他应用 AgentWeb 3.0.0 加入。
    ...
    public boolean onMenuItemClick(MenuItem item) {
        ...
        if (i == R.id.refresh) {
            ...
            if (mAgentWeb != null) {
                toCopy(mAgentWeb.getWebCreator().getWebView().getUrl());
            }
...
//            AgentWebConfig.clearDiskCache(this.getContext());
     ...
     * @param text
     */
    private void toCopy(String text) {
